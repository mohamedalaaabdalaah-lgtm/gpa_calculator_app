...
def check_scale(scale_data):
    ...
    for grade, points in scale_data.items():
        if not isinstance(grade, str) or not isinstance(points, (int, floa)):
            return False
        if points < 0:
            return False
...
def get_grade_points(grade, scale_typs):
    grade = grade.upper()

    if scale_type == "standard" :
        scale = STANDARD_3_3_SCALE

    elif scale_typs == "half_point":
        scale = HALF_POINT_3_5_SCALE
